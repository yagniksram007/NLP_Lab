...
void getTag(char word[], char tag[]) {
    ...
    for (const string &adv : adverb) {
        if (strcmp(word, adv.c_str()) == 0) { // if adverb
            strcpy(tag, "JJ");
            return;
        }
    }
...
int main() {
    ...
    char word[50];
    char tag[10];
    char *token = strtok(sentence, " ."); // tokenization
...
// Output:
// Tagged Sentence:
// the/DET cat/NN is/PRP very/JJ fat/NN sleeping/VBG on/PRP the/DET mat/NN
